...
export const updateId = (req: Request, res: Response): void => {
  const { id } = req.params
try {
  const deleteEntri = diaries.findIndex(d => d.id === Number(id))
    if (deleteEntri === -1) {
    ...
    const updateData: diarieWithoutId = req.body
    diaries[deleteEntri] = { ...diaries[deleteEntri], ...updateData };
    res.status(200).send('Diario actualizado con éxito');
} catch (error) {
  if (error instanceof Error) {
    console.error('Error al consultar los Diarios: ', error)
    res.status(500).send(error.message)
